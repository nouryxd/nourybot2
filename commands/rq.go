...
func HandleRq(channel string, username string, streamer string) {
	resp, err := http.Get(fmt.Sprintf("https://api.ivr.fi/logs/rq/%s/%s", streamer, username))
	if err != nil {
		log.Fatalln(err)
	}
